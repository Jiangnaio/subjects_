...
CONFIG = {
    ...
    "train_file": "qwen3_embedding_train_dedup.json",
...
def get_embeddings(model, input_ids, attention_mask, dtype=None):
    ...
    embeddings =average_pool(outputs.last_hidden_state, attention_mask)  # [B, D]
    return F.normalize(embeddings, p=2, dim=1)

def compute_qwen3_info_nce_loss(query_embeds, text_embeds, labels, query_indices, temperature=0.05):
    """
    稳定版：使用 log_softmax + NLL，避免手动 logsumexp 不稳定
    """
    ...
    for q_idx in unique_query_indices:
        ...
        loss = -torch.mean(pos_log_probs)  # 平均每个正样本的负对数似然
        ...
        for batch in tqdm(dataloader, desc="Evaluating", leave=False):
            ...
            try:
                ...
                if CONFIG["loss_type"] == "info_nce":
                    ...
                    # loss =compute_improved_contrastive_loss_with_regularization(
                    #     query_embeds, text_embeds, labels, query_indices,
                    # )
