# 训练Snowflake/snowflake-arctic-embed-l-v2.0的脚本，如果要训练Snowflake/snowflake-arctic-embed-m-v2.0，代码可能需要更改
import os
import random
import math
...
CONFIG = {
    ...
    "max_positives": 5,   #8,3 --> 10,5
    "max_hard_negatives": 0, # 硬负样本
    "temperature": 0.05,  # Qwen3 官方温度值
    ...
    "loss_type": "info_nce",  # 可选"info_nce"或"mlcl"
}
...
class ContrastiveDataCollator:
    ...
    def __call__(self, features):
        ...
        for i, f in enumerate(features):
            for pos_topic in f["positive_topics"]:
                all_texts.append(pos_topic)
                labels.append(1)
                query_indices.append(i)
            for neg_topic in f["negative_topics"]:
                all_texts.append(neg_topic)
                labels.append(0)
                query_indices.append(i)
            for hard_neg_topic in f["hard_negative_topics"]:
                all_texts.append(hard_neg_topic)
                labels.append(0)
                query_indices.append(i)
...
def load_datasets(tokenizer):
    ...
    eval_indices = random.sample(range(len(eval_dataset)), min(1000, len(eval_dataset)))
...
# ✅ 替换为 Qwen3 官方 InfoNCE 损失
def compute_qwen3_info_nce_loss(query_embeds, text_embeds, labels, query_indices, temperature=0.05):
    """
    Qwen3 Embedding 官方损失函数 (arXiv:2506.05176v3)
    - 支持多正样本
    - 使用温度缩放
    - 无 margin / hinge / diversity 正则
    """
    cos_sim = torch.matmul(query_embeds, text_embeds.T) / temperature  # [Q, T]
    unique_query_indices = torch.unique(query_indices)
    total_loss = 0.0
    valid_count = 0

    for q_idx in unique_query_indices:
        mask = (query_indices == q_idx) # 找出该 query 对应的所有 texts
        logits = cos_sim[q_idx, mask]    # [M]
        q_labels = labels[mask]          # [M], 1=positive, 0=negative

        pos_mask = (q_labels == 1)
        if pos_mask.sum() == 0:
            continue

        # Numerator: log-sum-exp of positives
        pos_logits = logits[pos_mask]
        numerator = torch.logsumexp(pos_logits, dim=0)

        # Denominator: log-sum-exp of all samples
...
def compute_mlcl_loss(query_embeds, text_embeds, labels, query_indices, temperature=0.05):
    """
    多标签对比损失 (Multi-Label Contrastive Loss) - 用于 Warm-up 阶段
    L(i) = -log[ sum_{j in P_i} exp(sim(q_i, t_j)/τ) / sum_{k=1}^T exp(sim(q_i, t_k)/τ) ]
    
    Args:
        query_embeds: [Q, D] - 查询嵌入
        text_embeds: [T, D] - 文本嵌入（包含所有正负样本）
        labels: [T] - 标签 (1=正样本, 0=负样本)
        query_indices: [T] - 每个文本对应的查询索引
        temperature: 温度参数 τ
    """
    device = query_embeds.device
    # 计算相似度矩阵 [Q, T]
    cos_sim = torch.matmul(query_embeds, text_embeds.T) / temperature
    ...
    for q_idx in unique_query_indices:
        # 找出该查询对应的所有文本
        mask = (query_indices == q_idx)  # [T] -> 布尔掩码
        logits = cos_sim[q_idx, mask]    # [M] - 该查询对应的所有文本的 logits
        q_labels = labels[mask]          # [M] - 对应的标签
        ...
        # 分母：所有文本的 log-sum-exp（当前查询对应的所有文本）
        denominator = torch.logsumexp(logits, dim=0)
...
def evaluate(model, dataloader, device, dtype, temperature=0.05):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Evaluating", leave=False):
            ...
            query_indices = batch["query_indices"].to(device, non_blocking=True)
            ...
            try:
                ...
                if CONFIG["loss_type"] == "info_nce":
                    loss = compute_qwen3_info_nce_loss(
                        query_embeds, text_embeds, labels, query_indices,
                        temperature=CONFIG["temperature"]
                    )
                elif CONFIG["loss_type"] == "mlcl":
                    loss = compute_mlcl_loss(
                        query_embeds, text_embeds, labels, query_indices,
                        temperature=CONFIG["temperature"]
                    )
                ...
                del labels, query_indices, query_embeds, text_embeds, loss
...
if __name__ == "__main__":
    ...
    collator = ContrastiveDataCollator(tokenizer, max_length=CONFIG["max_length"])
    train_dataloader = DataLoader(train_dataset, batch_size=CONFIG["batch_size"], shuffle=True, collate_fn=collator, drop_last=True, pin_memory=True)
        ...
        for step, batch in batch_iter:
            ...
            try:
                with torch.autocast(device_type="cuda", dtype=dtype):
                    ...
                    if CONFIG["loss_type"]=="info_nce":
                        loss = compute_qwen3_info_nce_loss(
                            q_emb, t_emb, labels, query_indices,
                            temperature=CONFIG["temperature"]
                        )
                    elif CONFIG["loss_type"]=="mlcl":
                        loss = compute_mlcl_loss(
                ...
                del query_input_ids, query_attention_mask, text_input_ids, text_attention_mask, labels, query_indices
    ...
    create_training_plots(training_logs, CONFIG["plot_dir"])
    print("✅ Full fine-tuning with Qwen3 InfoNCE loss completed.")
