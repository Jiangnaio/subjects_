...
# ================= 配置 =================
CONFIG = {
    "sample_path": "./qwen3-rerank_data_lora/rerank_train_samples_batched.json",
    "label_emb_path": "./qwen3-rerank_data_lora/label_embeddings.pt",
    "query_emb_path": "./qwen3-rerank_data_lora/query_embeddings.pt",
    "batch_size": 32,
    "epochs": 100,
    "lr": 1e-4,
    "weight_decay": 1e-5,
    "device": "cuda" if torch.cuda.is_available() else "cpu",
    "model_save_path": "./qwen3-rerank_data_lora/rerank_model_contextual.pth",
...
# ================= 数据集 =================
# 如果张量在保存之前在GPU上，在加载张量时，默认也是保存在GPU上，进而导致该代码报错
...
def train():
    ...
    for epoch in range(args.epochs if args.epochs is not None else CONFIG["epochs"]):
        model.train()
        total_loss = 0.0
