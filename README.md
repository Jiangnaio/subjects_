2025.12.26 加入熵正则化，虽然能够使训练的损失函数更平滑，但训练效果更差（训练0.6B参数）；训练4B参数仍然出现效果不升反降。参考Qwen3-Embedding使用的损失函数，重写损失函数，对qwen3-embedding-0.6b先前训练的epoch-1的基础上，进行全参数微调，训练1500 steps(batch_size=16) 效果明显提升, 在dev随机1000样本数据集上recall@50从0.6944提升到0.7722(zero-shot为0.6434), 训练2000 steps 有所下降为0.7529。 
