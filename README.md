2026.1.28 基于LightXML的思想，训练LightXML head，输出相对于未经过训练的嵌入模型的纯向量检索有明显提升，使用经过训练的嵌入模型召回率更好，但比起经过训练的嵌入模型纯向量检索召回率略差，但是两个模型的结果相融合，效果明显提升。 与此同时，将top-500的候选标签的编码向量与query的编码向量用于训练轻量级重排器（双层MLP），对召回率也有明显提升。
